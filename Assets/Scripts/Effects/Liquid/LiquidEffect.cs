// From https://www.patreon.com/posts/quick-game-art-18245226
...
public class LiquidEffect : MonoBehaviour
{
    ...
    [SerializeField] private AnimationCurve fillingCurve = AnimationCurve.Linear(0f, 1f, 1f, 0f);
    [SerializeField] private AnimationCurve emptyingCurve = AnimationCurve.Linear(0f, 0f, 1f, 1f);
    ...
    private Coroutine emptyingFillingCoroutine;

    public bool IsFilled => fillingCurve.keys[fillingCurve.keys.Length - 1].value == fillAmount;
    ...
    public void EmptyOrFill()
    {
        if (emptyingFillingCoroutine != null)
        {
            return;
        }

        emptyingFillingCoroutine = StartCoroutine(EmptyOrFillCoroutine());
    }

    public IEnumerator EmptyOrFillCoroutine()
    {
        AnimationCurve curve = IsFilled ? emptyingCurve : fillingCurve;
        ...
        while (progress < 1f)
        {
            ...
            float evaluationTime = endTime * progress;
            fillAmount = curve.Evaluate(evaluationTime);

            yield return null;
        }

        emptyingFillingCoroutine = null;
    }
